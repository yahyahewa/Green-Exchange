import React from "react";

function leftAside({ name, donate, recive, img }) {
  img =
    "data:image/png;base64,";
  return (
    <article className="w-[30%] flex flex-col items-start justify-center gap-y-4">
      <div className="border w-[90%] rounded-lg p-2 text-slate-600">
        <h1 className="text-center text-xl mb-2 capitalize text-slate-700 font-bold">
          {name}
        </h1>
        <div className="flex items-center justify-evenly">
          <img src={img} className="w-14 rounded-full border" />
          <div className="flex">
            <div className="flex flex-col border-r pr-2">
              <span className="text-center font-bold">{donate}</span>
              <span className="text-sm text-center">donate</span>
            </div>
            <div className="flex flex-col pl-2">
              <span className="text-center font-bold">{recive}</span>
              <span className="text-sm text-center">donate</span>
            </div>
          </div>
        </div>
      </div>
      <div
        className="border w-[90%] rounded-lg p-2 text-slate-600 h-[100px]
        font-bold flex justify-center items-center"
      >
        Add Product <i class="fa-solid fa-plus fa-lg ml-2"></i>
      </div>
    </article>
  );
}

export default leftAside;
